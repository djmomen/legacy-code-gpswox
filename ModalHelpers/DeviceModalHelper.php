...
class DeviceModalHelper extends ModalHelper
{
    ...
    public function changeActive()
    {
        ...
        if (array_key_exists('group_id', $this->data)) {
            $filter = empty($this->data['s']) || !config('addon.sidebar_group_selection_filter')
                ? null
                : Device::search($this->data['s'])->select('id')->toBase();

            $updateCount = $this->deviceUsersService->setVisibleGroups($this->user, $this->data['group_id'], $active, $filter);
        } else {
