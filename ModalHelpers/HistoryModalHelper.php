...
use App\Transformers\TraccarPosition\TraccarPositionFullTransformer;
use Carbon\Carbon;
...
class HistoryModalHelper extends ModalHelper {
    ...
    public function getStartEnd()
    {
        ...
        $from = Formatter::time()->reverse($this->data['from_date'] . ' ' . $this->data['from_time']);
        $to = Formatter::time()->reverse($this->data['to_date'] . ' ' . $this->data['to_time']);

        $isExtended = request('strategy') === 'extended';

        $startHigh = $device->positions()->where('time', '>=', $from)->orderliness('ASC')->first();
        $endLow = $device->positions()->where('time', '<=', $to)->orderliness('DESC')->first();

        if ($isExtended) {
            $startLow = $device->positions()->where('time', '<', $from)->orderliness('DESC')->first();
            $endHigh = $device->positions()->where('time', '>', $to)->orderliness('ASC')->first();

            $start = $this->getClosestToDatetimePosition($from, $startHigh, $startLow);
            $end = $this->getClosestToDatetimePosition($to, $endHigh, $endLow);
        } else {
            $start = $startHigh;
            $end = $endLow;
        }
        ...
        return [
            'start' => $start
                ? FractalTransformer::setSerializer(WithoutDataArraySerializer::class)->item($start, TraccarPositionFullTransformer::class)->toArray()
                : null,
            'end' => $end
                ? FractalTransformer::setSerializer(WithoutDataArraySerializer::class)->item($end, TraccarPositionFullTransformer::class)->toArray()
                : null,
        ];
    }

    private function getClosestToDatetimePosition(string $datetime, ?TraccarPosition $a, ?TraccarPosition $b): ?TraccarPosition
    {
        if (!$a && !$b) {
            return null;
        }
        ...
        $diffA = abs(strtotime($a->time) - $target);
        $diffB = abs(strtotime($b->time) - $target);
