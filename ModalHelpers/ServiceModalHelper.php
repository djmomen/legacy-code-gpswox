...
class ServiceModalHelper extends ModalHelper {
    ...
    public function paginatedBase($device_id = NULL)
    {
        if (is_null($device_id)) {
            $device_id = array_key_exists('device_id', $this->data) ? $this->data['device_id'] : request()->route('device_id');
        }

        return DeviceService::when(!$device_id, fn (Builder $query) => $query->userAccessible($this->user))
            ->when($device_id, fn (Builder $query) => $query->where('device_id', $device_id))
            ->toPaginator($this->data['limit'] ?? 10, 'id', 'DESC');
    }

    public function paginated($device_id = NULL)
    {
        $services = $this->paginatedBase($device_id);
