...
class Device extends AbstractEntity implements DisplayInterface, FcmTokenableInterface, ChattableInterface
{
    ...
    const STATUS_NEVER_CONNECTED = 'never_connected';
    ...
    {
        return $this->belongsToMany(DeviceGroup::class, 'user_device_pivot', 'device_id', 'group_id')
            ->withPivot('user_id');
    }
    ...
    public function group()
    {
        ...
        return $this->groups()->wherePivot('user_id', $userId);
    }
    ...
    public function getLastDriverChange($time = null): ?object
    {
        return DB::table('user_driver_position_pivot')
            ->where('device_id', $this->id)
            ->when($time, fn (QueryBuilder $query) => $query->where('date', '<=', $time))
            ->orderBy('date', 'DESC')
            ->first();
    }

    private function setCurrentDriver($driver, $rfid, $time): void
    {
        $prev = $this->getLastDriverChange();

        if ($time && $prev && (string)$time <= $prev->date) {
            return;
        }

        if ($this->current_driver_id == $driver?->id) {
            return;
        }
    ...
    private function logDriverChange($driver, $rfid, $time): void
    {
        $prev = $this->getLastDriverChange($time);

        if ($prev && $prev->driver_id == $driver?->id) {
            return;
        }
    ...
    public function changeDriver($driver, $rfid = null, $time = null, $withoutAlerts = false)
    {
        ...
        $this->setCurrentDriver($driver, $rfid, $time);
        $this->logDriverChange($driver, $rfid, $time);
