@if (!empty($events))
    @php /** @var \Tobuli\Entities\Event $item */ @endphp
    @foreach ($events as $item)
        <tr data-event-id="{!!$item->id!!}" onClick="app.events.select({!!$item->id!!});">
            ...
            <td>
                @php
                    $eventActions = [
                        'delete' => Auth::user()->can('remove', $item),
                        'edit_alert' => $item->alert ? Auth::user()->can('edit', $item->alert) : false,
                        'create_call_action' => Auth::user()->perm('call_actions', 'edit') && Auth::user()->perm('events', 'view'),
                    ]
                @endphp

                @if (array_filter($eventActions))
                <div class="btn-group dropleft droparrow"  data-position="fixed">
                    ...
                    <ul class="dropdown-menu">
                        ...
                        @if ($eventActions['create_call_action'])
                            <li>
                                <a href="javascript:" data-url="{{ route('call_actions.create', ['device_id' => $item->device_id, 'event_id' => $item->id]) }}" data-modal="call_action_create">
