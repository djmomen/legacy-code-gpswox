...
class AsaasGateway extends PaymentGateway implements PaymentGatewayInterface
{
    ...
    public function isConfigCorrect(Request $request): bool
    {
        ...
        if (empty($response['data'][0])) {
            $this->handleException(new PaymentsConfigurationException('Auth check failed'));
        }

        $data = $response['data'][0];

        if (isset($data['apiVersion']) && $data['apiVersion'] === 3 && !empty($data['hasAuthToken'])) {
            return true;
        }

        if (empty($data['authToken']) || $data['authToken'] !== $this->config['access_token']) {
