...
abstract class EnityLoader
{
    ...
    public function get()
    {
        ...
        foreach ($this->appends as $append) {
            $items->appends([
                $append => $this->getRequestParam($append),
            ]);
        }
    ...
    protected function getItems()
    {
        ...
        $query->search($this->getRequestParam(self::KEY_SEARCH));

        if ($id = $this->getRequestParam('selected_id')) {
            $sequence = $this->parseSequence();
    ...
    protected function parseSequence() : FilterValuesSequence
    {
        if (empty($this->sequence)) {
            ...
            foreach ($this->getRequestParam($this->request_key, []) as $select) {
    ...
    */
    ...
    protected function getRequestParam(int|string $key, mixed $default = null): mixed
    {
        if (isset($this->input)) {
